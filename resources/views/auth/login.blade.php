                        ...
                        <form method="POST" action="{{ route('login') }}">
                            ...
                            <div class="mb-3 position-relative">
                                <input type="password" class="form-control" name="password" id="password"
                                       placeholder="{{ __('Entrez votre mot de passe') }}" aria-label="Password"
                            ...
                            <div class="text-center">
                                <button type="submit"
                                        class="btn bg-gradient-info w-100 mt-4 mb-0">{{__('Se connecter')}}</button>
                            </div>
                        </form>
                    </div>
                        ...
                        @if (Route::has('password.request'))
                            <small class="text-muted">{{__('Mot de passe oublié ?')}} {{__('Réinitialiser votre mot
                                ...
                                <a href="{{ route('password.request') }}"
                                   class="text-info text-gradient font-weight-bold">{{__('ici')}}</a>
                            </small>
                        @endif
                        <p class="mb-4 text-sm mx-auto">
                            {{__('Vous n’avez pas de compte ?')}}
                            <a href="{{ route('register')}}"
                               class="text-info text-gradient font-weight-bold">{{__('S’inscrire')}}</a>
                        </p>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="oblique position-absolute top-0 h-100 d-md-block d-none me-n8">
                    <div class="oblique-image bg-cover position-absolute fixed-top ms-auto h-100 z-index-0 ms-n6"
                         style="background-image:url('{{url('assets/img/bg/pngtree.jpg')}}')"></div>
                        </div>
                    </div>
                </div>
            </div>
